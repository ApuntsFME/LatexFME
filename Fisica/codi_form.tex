...
\emph{Si l'eix és fix}: $E_\text{c} = \frac{1}{2} I_e \omega^2 $\\
\emph{Si l'eix és mòbil}: $E_{\text{c}} = \frac{1}{2} I_\text{CM}\omega^2 + \frac{1}{2}  M\vec{v}_{\text{CM}}^2$.\\
...
\emph{Canvi m. lineal}: $\Delta \vec{P} = \vec{I}$.\\
\emph{Canvi m. angular}: Si $A$ és un punt fix o el CM, $\Delta \vec{L}_A = \vec{J}_A$.\\
