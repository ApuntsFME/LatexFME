...
\begin{proof}
    \begin{enumerate}[i)]
        \item[]
        \item Com que els $A_i$ són disjunts i $\bigcup_{i \in I}{A_i} = \Omega$, aleshores $\forall B \in \Asuc$,
        $B = \bigcup_{i\in I}{B \cap A_i}$, i la unió és disjunta:
        \[
        ...
        \item
        \[
            p(A_i|B) \sum_{j \in I}{p(B|A_j)p(A_j)} \stackrel{i)}{=} p(A_i|B)p(B) = \frac{p(B\cap A_i)}{p(B)}p(B) =
            p(B \cap A_i) \implies
        \]\[
            \implies p\lp A_i\mid B\rp=\frac{P\lp B\mid A_i\rp p\lp A_i\rp}{\sum_{i\in I} p\lp B\mid A_i\rp p\lp A_i\rp}
        \]
    \end{enumerate}
\end{proof}
