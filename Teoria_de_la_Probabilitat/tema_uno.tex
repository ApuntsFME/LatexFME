...
\begin{defi}[espai!de probabilitat]\index{espai!mostral((tlab))}\index{conjunt!d'esdeveniments((tlab))}\index{conjunt!de successos((tlab))}\index{funció!de probabilitat((tlab))}
    Un espai de probabilitat és un espai de mesura format per la terna $(\Omega,\Asuc, p)$ tal que $p(\Omega)=1$. Diem que 
    \begin{itemize}
...
\begin{obs}
    ...
    \begin{enumerate}[i)]
        ...
        \item Si $\lc A_i\rc _{i\in\n}\subseteq \Asuc$, aleshores $\bigcup\limits_{i\in\n}{A_i} \in \Asuc$.
    \end{enumerate}
...
\begin{proof}
    \begin{enumerate}
        ...
        \item Conseqüència directa de \ref{item:esp_prob_2} usant que $\Asuc = A\cup \comp{A}$.
...
hi ha estimacions per a $p(\bigcup_{i=1}^{r}{A_i})$ com es veu al teorema següent.
...
\begin{teo}[Desigualtats de Bonferroni]
    ...
    \]
    això és, $S_1 = \sum{p(A_i)}$, $S_2 = \sum\limits_{i \neq j}p\lp A_i \cap A_j\rp,\dots$. Aleshores:
    \begin{enumerate}[i)]
         \item Si $t$ és parell,
            \[p\lp\bigcup_{i=1}^{r}{A_i}\rp \geq \sum_{i=1}^{t}{(-1)^{i+1}S_i}\]
         \item Si $t$ és senar,
            \[p\lp\bigcup_{i=1}^{r}{A_i}\rp \leq \sum_{i=1}^{t}{(-1)^{i+1}S_i}\]
    \end{enumerate}
\end{teo}
...
\begin{example}
    \begin{enumerate}
     ...
     Prenem un conjunt numerable $\Omega =\left\{a_i\right\}_{i\geq 1}$. Prenem $\Asuc = \Pa\lp \Omega\rp$
     (que és una $\sigma$-àlgebra). Per a definir la probabilitat sobre $\lp\Omega,\,\Asuc\rp$ prenem una successió 
     $\left\{p_i\right\}_{i\geq 1}$ tal que $0\leq p_i\leq 1$ que compleix que $\forall i,\, p\lp a_i\rp = p_i$ i 
     $\sum p_i=1$. Per tant, per a qualsevol element $A\in\Asuc$, tenim que
     \[
	...
	10010111110010\dots 
     \end{gather*}
      ...
      \[
        ...
        \end{aligned}.
      \]
      on $0.a_1a_2\dots$ és un nombre en binari. No és una bijecció completa ja que hi ha elements diferents que van
...
\begin{obs}
    Sigui $\lp \Omega, \Asuc, p\rp$ un espai de probabilitat i sigui $B \in \Asuc \tq p\lp B\rp > 0$. Aleshores, l'aplicació
    \begin{align*}
...
\begin{prop}
    ...
    \begin{enumerate}[1)]
        ...
        \item Fórmula de Bayes:
            \[
                p\lp A_i\mid B\rp=\frac{P\lp B\mid A_i\rp p\lp A_i\rp}{\sum\limits_{j\in I} p\lp B\mid A_j\rp p\lp A_j\rp}, \quad \forall B\in \Asuc \text{ amb } p\lp B\rp>0.
            \]
    \end{enumerate}
\end{prop}
\begin{proof}
...
\end{problema}
\begin{sol}
    Sigui $A_k$ el succés ``el jugador, començant amb capital $k$, perd''. Condicionem $A_k$ a la primera tirada de la moneda i 
    definim $B$ com el succés ``la primera tirada surt cara''. Aleshores,
    \begin{gather*}
      p(A_k) = p(A_k|B)p(B) + p(A_k|\comp{B})p(\comp{B}) = \frac{p(A_k|B)}{2} + \frac{p(A_k|\comp{B})}{2}\\
...
\begin{defi}[esdeveniments independents]
    Sigui $\lp \Omega, \Asuc, p\rp$ un espai de probabilitat, sigui $I$ un conjunt finit o numerable i sigui $\lc A_i\rc_{i\in I} \subseteq \Asuc$. 
    Diem que els esdeveniments $A_i$ són independents si $\forall J\subseteq I$ amb $\abs{J}\in\n$ es té que
    \[
...
\begin{example}
    \begin{enumerate}[1.]
        ...
        \item No tenim independència si només les interseccións dos a dos compleixen que la probabilitat de la intersecció és el producte de probabilitats.
        \item $A$ i $B$ independents $\iff A$ i $\comp{B}$ independents, ja que
        \begin{gather*}
         p( A\cap\comp{B})=p\lp A\rp-p\lp A\cap B\rp= p\lp A\rp-p\lp A\rp\lp B\rp\\
	 =p\lp A\rp\lp p\lp B\rp + p(\comp{B}) -p\lp B\rp\rp=p\lp A\rp p(\comp{B})
        \end{gather*}
...
\begin{obs}
  Els dos límits pertanyen a $\Asuc$ ja que són unió i intersecció numerable de sucessos.
\end{obs}
...
\end{prop}
\begin{proof}
    Definim $B_n=\bigcup\limits_{k\geq n}A_k$ i $C_n=\bigcap\limits_{k\geq n}A_k$. Observem que $\lc B_n\rc_{n\geq 1}$ és decreixent i que $\lc C_n\rc_{n\geq 1}$ és creixent. Naturalment tenim que, $C_n\subseteq A_n\subseteq B_n$, $\limsup A_n = \bigcap\limits_{n\geq 1} B_n$ i $\liminf A_n = \bigcup\limits_{n\geq 1} C_n$.
    ...
    \]
    Al darrer pas hem utilitzat el fet que $p\lp \bigcap\limits_{k\geq n} A_k\rp\leq p\lp A_n\rp$. Anàlogament, tenim que $\limsup p\lp A_n\rp\leq p\lp\limsup A_n\rp$. Així doncs,
    \[
    ...
    \]
\end{proof}
\begin{teo}[Lema de Borel-Cantelli]
    Sigui $\lp\Omega,\Asuc,p\rp$ un espai de probabilitat i siguin $\left\{ A_n\right\}_{n\geq1} \subseteq \Asuc$. Aleshores,
    \begin{enumerate}[i)]
...
\end{teo}
\begin{proof}
    Anomenem $A=\limsup A_n=\bigcap\limits_{n\geq 1}\bigcup\limits_{k\geq n} A_k$.
    \begin{enumerate}[i)]
        \item Sabem que
            \[
                0\leq p\lp A\rp\leq p\lp\bigcup_{k\geq n}A_k\rp\leq\sum_{k\geq n} p\lp A_k\rp ,\,\forall n\in\n
            \]
            i que $\sum\limits_{n\geq 1} p\lp A_n\rp<\infty$, de manera que $\lim \sum\limits_{k\geq n} p\lp A_k\rp=0$ i immediatament deduïm que $p\lp A\rp=0$.
        \item Observem primer que $\comp{A}=\comp{\bigcap\limits_{n\geq 1}\bigcup\limits_{k\geq n} A_k}=\bigcup\limits_{n\geq 1}\bigcap\limits_{k\geq n} \comp{A}_k=\liminf\comp{A}_n$. Veurem que $p( \comp{A})=0$. Calculem $p\lp \bigcap\limits_{m\geq n} \comp{A}_m\rp$.
        \begin{align*}
            0&\leq p\lp \bigcap\limits_{m\geq n} \comp{A}_m\rp = \lim_{r\to\infty} p\lp \bigcap\limits_{m= n}^r \comp{A}_m\rp = \lim_{r\to\infty} \prod_{m= n}^r \lp p\lp\comp{A}_m\rp\rp =\\
            &= \lim_{r\to\infty} \prod_{m= n}^r \lp 1-p\lp A_m\rp\rp \leq \lim_{r\to\infty} \prod_{m= n}^r \lp e^{-p\lp A_m\rp}\rp =\\
            &=\lim_{r\to\infty} e^{-\sum\limits_{m= n}^r p\lp A_m\rp} = 0,
        \end{align*}
        de manera que $p\lp \bigcap\limits_{m\geq n} \comp{A}_m\rp=0, \,\forall n\in\n$.
        Finalment,
        \[
            0\leq p\lp\comp{A}\rp=p\lp\bigcup\limits_{n\geq 1}\bigcap\limits_{m\geq n} \comp{A}_m\rp\leq\sum_{n\geq 1}p\lp\bigcap\limits_{m\geq n} \comp{A}_m\rp=0+0+\cdots=0,
        \]
