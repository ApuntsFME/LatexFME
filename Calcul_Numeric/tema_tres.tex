...
\begin{proof}
    ...
    Tenemos que $A$ es
    \begin{align*}
        A &= \int_{x_0}^{x_n} S''\left( g''-S'' \right) \dif x = \\ &= \cancel{\left[ S''\left( g' - S' \right) \right]_{x_0}^{x_n}} - \int_{x_0}^{x_n} S'''\left( g'-S' \right) \dif x = \\
        &= -\sum_{i=0}^{n-1} \int_{x_i}^{x_{i+1}} S'''\left( g'-S' \right) \dif x  = \\ &= -\sum_{i=0}^{n-1} S'''_i\cancel{\left[ g-S \right]_{x_i}^{x_{i+1}}} = 0.
    \end{align*}
