...
\u{S\`eries geom\`etriques}: $\sum_{n \geq 1} \alpha^n$ conv. sii $\alpha \in (-1, 1)$, oscil·lant sii $\alpha=-1$, div. altrament.

\section{Sèries n. positius}
Dins aquest apartat les successions són totes de termes positius. \\
\u{C. comp. dir.}: $a_n \leq b_n \forall n \geq n_0 \implies \sum_{n=n_0}^\infty a_n \leq \sum_{n=n_0}^\infty b_n \implies (\sum b_n$ conv. $\implies \sum a_n$ conv.$)$ i $(\sum a_n$ div. $\implies \sum b_n$ div.$)$. \\
\u{C. comp. al límit}: $\lim\frac{a_n}{b_n}=l\in[0,+\infty]$. Si $l < \infty, \, \sum b_n$ conv. $\implies \sum a_n$ conv.. Si $l > 0, \, \sum a_n$ conv. $\implies \sum b_n$ conv.\\
...
\underline{C. de la integral}: $a_n = f(n), f \geq 0$ int. i decreixent, $\int_{M}^{\infty} f$ convergeix $\iff \sum a_k$ convergeix i $\sum_{M}^{\infty} = \sum_{M}^{N-1} + \int_{N}^{\infty} f + \varepsilon_N$, $\varepsilon_N \in [0,a_N]$. \\
\underline{C. logarítmic}: $\lim\limits_{n \to \infty} \frac{-\log{a_n}}{\log{n}} = L \implies$ ($L > 1$ conv.) i  ($L < 1$ div.). \\
\underline{C. condensació}: $a_n$ decreixent, $a_n \geq 0$, $\sum a_n$ convergent $\iff \sum 2^na_{2^n}$ convergent.
...
\u{Sèrie alternada}: un pos., un neg., ... \\
...
\u{Domini de conv.}: $\lp -R, R\rp$, on $R$ és radi de conv.. És possible que convergeixi als exterms. \\
\u{T. Cauchy-Hadamard}: Sigui $\sum a_nx^n, R$ ve donada per $\frac{1}{R} = \lim \sup \abs{a_n}^{\nicefrac{1}{n}}$. La sèrie de potències és abs. conv. si $\abs{x} < R$ i div. si $\abs{x} > R$. Si $\abs{x} = R$ no sabem res. \\
...
\u{Suma inf. del rect.}: $m_R = \inf_{x\in R} f(x), \, s(f; \Pa) = \sum_R m_R \vol(R)$. \\
\u{Suma sup. del rect.}: $M_R = \sup_{x\in R} f(x), \, S(f; \Pa) = \sum_R M_R \vol(R)$. \\
...
\ci Sigui $f\colon U\to \real$ classe $\C^1$ o lipschitziana, $z \subset U$ mesura nul·la, llavors $f(z) \subset \real^n$ té mesura nul·la.
...
\u{Regió elemental}: A $\real$ és un interval compacte. Si no és de la forma $\{(x,y) \in \real^{n-1} \times \real \vert x \in D$, $\phi(x) \leq y \leq \psi(x)\}$, on $D \subset \real^{n-1}$
            és regió elemental i $\phi \leq \psi \colon D \to \real$ contínues.
\section{Canvi de variables}

\ci Sigui $V\subset\real^n$ obert, $\varphi\colon V\to\real^n$ injectiva, classe $\C^1$ amb $\det \dif \varphi( y ) \neq 0, \forall y \in V$. Sigui $U = \varphi( V ) (\varphi\colon V\to U$ difeo. classe $\C^1)$. Si $f\colon U\to \real$ int., $\int_U f = \int_V( f \circ \varphi )\abs{\det \dif \varphi}$.
...
\ci $\gamma \colon I\to \real^n$ classe $\C^1 \implies$ rectificable i $L(\gamma) = \int_I \|\gamma'\|$.
...
\quad \ci $\rot ( f \vec{G} ) = f \rot \vec{G} + \grad f \times \vec{G}$, \\
\quad \ci $\diver ( f \vec{G} ) = f \diver \vec{G} + \grad f \cdot \vec{G}$, \\
\quad \ci $\diver ( \vec{F} \times \vec{G} ) = \vec{G} \cdot \rot \vec{F} - \vec{F} \cdot \rot \vec{G}$. \\
\u{T. Schwarz}: $f, \vec{F} \in \C^2 \implies \rot( \grad f ) = 0, \diver ( \rot \vec{F} ) = 0$. \\
...
\u{T. fon. c\`alcul}: (o gradient) $W \subseteq \real^n$ obert, $f \colon W \to \real$ classe $\C^1$. $C$ corba regular orientada classe $\C^1, \tq \bar{C} \subset W$ compacte $\implies \int_C \grad f \dif \vec{l} = \int_{\partial C} f = f( x_1 ) - f( x_0 ), \partial C = \setb{x_0, x_1}$ (orientada $x_0\to x_1$). $\partial C = \emptyset\implies \int_{\partial C} f = 0$. \\
\u{T. Kelvin-Stokes}: (o rotacional) $W \subseteq \real^3$ obert, $\vec{F} \colon W \to \real^3$ classe $\C^1$, $M \subset W$ superficie orientada classe $\C^2 \tq \bar{M}$ compacte, $\bar{M} \subset W$, $\partial M$ $^\ast$ $\implies \int_M \rot \vec{F} \cdot\dif \vec{S} = \int_{\partial M} \vec{F} \cdot\dif \vec{l}$. \\
\u{T. Gauss-Ostrogradski}: (o diverg\`encia) $W \subset \real^3$ obert, $\vec{F} \colon W \to \real^3$ classe $\C^1$, $B \subset W$ obert t.q. $\bar{B}$ compacte, $\bar{B} \subset W$, $\partial B$ $^\ast$ $\implies \int_B \diver \vec{F} \dif V = \int_{\partial B} \vec{F} \cdot\dif \vec{S}$.
...
\u{T. Green}: $U \subseteq \real^2$ obert, $\vec{F} \colon U \to \real^2$ classe $\C^1$, $M \subset U$ obert t.q. $\bar{M}\subset U$ compacte, $\partial M$ $^\ast$ $\implies \int_M ( \pdv{F_2}{x} - \pdv{F_1}{y} ) \dif x \dif y = \int_{\partial M} \vec{F}\cdot \dif \vec{l}$.
...
\u{Integral de $\omega$ al llarg de $\sigma$}: $\omega \in \Omega^k (\real^n),\, \sigma \colon D \subseteq \real^k \to \real^n, \implies \int_\sigma \omega := \int_D \sigma^\ast (\omega)$. \\
...
\subsection{Altres. Volums $n$-dimensionals}
\u{Esfera}: $S=4\pi r^2$, $V=\frac{4}{3}\pi r^3$. \\
\u{Piràmide}: $V = \frac{1}{3} bh$. \\
\u{Con}: $A = \pi r\sqrt{r^2 + h^2} + \pi r^2$. \\
\u{Casquet esfèric}: $A = 2\pi Rh$, $V = \frac{1}{3} \pi h^2 (3R-h)$. \\
\u{Parametrització el·lipse}: $(\frac{x}{a})^2 + (\frac{y}{b})^2 = c^2$ $\implies$ $\theta \mapsto c\cdot(a\cos(\theta), b\sin(\theta))$.
...
\ci $\int \sqrt{1-x^2} = \frac{1}{2}(\arcsin(x) + x\sqrt{1-x^2})+ C$. \\
...
\noindent\makebox[\linewidth]{\rule{\linewidth}{0.5pt}}
$^\ast$ amb orientació induïda (interior a l'esquerra) i punts frontera de $M$ regulars o conjunt de punts frontera singulars \'es finit. \\
\vspace{3pt}
\raggedleft
