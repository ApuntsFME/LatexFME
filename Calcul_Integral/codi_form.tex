...
\u{C. Raabe}: $(a_n)$ estr. pos. i $\exists\lim n\left(1-\frac{a_{n+1}}{a_n}\right) = L \implies (L > 1$ conv.$)$ i $(L<1$ div.$)$.\\
...
\u{Oscil·lació en $a\in X$}: $\omega\left( f,a \right) = \lim\limits_{r\to 0}\omega\left( f, B\left( a;r \right) \right) = \inf_{r>0} \omega\left( f, B\left( a; r \right) \right)$. \\
...
\u{Rotacional}: $\rot \vec{F} := \left( \pdv{F_3}{y} - \pdv{F_2}{z} \right)\hat{i} + \left( \pdv{F_1}{z} - \pdv{F_3}{x} \right) \hat{j} + \left( \pdv{F_2}{x} - \pdv{F_1}{y} \right) \hat{k}$. \\
...
\u{T. Schwarz}: $f, \vec{F} \in \mathcal{C}^2 \implies \rot\left( \grad f \right) = 0, \diver ( \rot \vec{F} ) = 0$. \\
...
\u{Laplaci\`a}: $\Delta f := \diver \left( \grad f \right), \Delta = \nabla^2 = \pdv[2]{}{x} + \pdv[2]{}{y} + \pdv[2]{}{z}$.

\section{F\'ormules int. teoria de camps}

\u{T. fon. c\`alcul}: (o gradient) $W \subseteq \real^n$ obert, $f \colon W \to \real$ classe $\C^1$. $C$ corba regular orientada clase $\C^1, \tq \bar{C} \subset W$ compacte $\implies \int_C \vec{\grad} f \dif \vec{l} = \int_{\partial C} f = f\left( x_1 \right) - f\left( x_0 \right), \partial C = \setb{x_0, x_1}$ (orientada $x_0\to x_1$). $\partial C = \emptyset\implies \int_{\partial C} f = 0$. \\
\u{T. Kelvin-Stokes}: (o rotacional) $W \subseteq \real^3$ obert, $\vec{F} \colon W \to \real^3$ classe $\C^1$, $M \subset W$ superficie orientada classe $\C^2 \tq \bar{M}$ compacto, $\bar{M} \subset W$, $\partial M$ amb orientació induïda (m\`a dreta) i punts frontera de $M$ regulars o conjunt de punts frontera singulars \'es finit $\implies \int_M \rot \vec{F} \dif \vec{S} = \int_{\partial M} \vec{F} \dif \vec{l}$. \\
\u{T. Gauss-Ostrogradski}: (o diverg\`encia) $W \subset \real^3$ obert, $\vec{F} \colon W \to \real^3$ classe $\C^1$, $B \subset W$ obert t.q. $\bar{B}$ compacte, $\bar{B} \subset W$, $\partial B$ amb orientació induïda (m\`a dreta) i punts frontera de $M$ regulars o conjunt de punts frontera singulars \'es finit $\implies \int_B \diver \vec{F} \dif V = \int_{\partial B} \vec{F} \dif \vec{S}$.
...
\u{T. Green}: $U \subseteq \real^2$ obert, $\vec{F} \colon U \to \real^2$ classe $\C^1$, $M \subset U$ obert t.q. $\bar{M}\subset U$ compacte, $\partial M$ amb orientació induïda (m\`a dreta) i punts frontera de $M$ regulars o conjunt de punts frontera singulars \'es finit $\implies \int_M \left( \pdv{F_2}{x} - \pdv{F_1}{y} \right) \dif x \dif y = \int_{\partial M} \vec{F} \dif \vec{l}$.
...
\ci $\alpha \wedge \beta = (-1)^{\abs{\beta}\abs{\alpha}} \alpha \wedge \beta$. \\
\ci $f \in \C^\infty \left( \real^n \right) = \Omega^0 \left( \real^n \right)$, podem construir $\dif f := \sum^n_{i=1} \frac{\partial f}{\partial x^i} \dif x^i \in \Omega^1 \left( \real^n \right)$. \\
...
\u{Pullback}: $F \colon \real^m \to \real^n$ classe $\C^\infty$, $g \in \C^\infty\left( \real^n \right)$, el pullback \'es $F^\ast(g) := g \circ F \in \C^\infty \colon \real^m \to \real$, $F^\ast$ \'es $\real$-lineal, $F^\ast(g\dif y^{i_1}\wedge\dots\wedge\dif y^{i_k}) := F^\ast (g) \dif F^\ast (y^{i_1}) \wedge \dots \wedge \dif F^\ast (y^{i_k})$. \\
