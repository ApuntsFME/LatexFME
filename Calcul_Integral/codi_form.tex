...
\underline{C. de la integral} $\left( a_n = f(n), f \geq 0 \text{ integ. i decreixent} \right)$ $\int_{M}^{\infty} f$ convergeix $\iff \sum a_k$ convergeix
i $\sum_{M}^{\infty} = \sum_{M}^{N-1} + \int_{N}^{\infty} f + \varepsilon_N$, $\varepsilon_N \in [0,a_N]$ \\
\underline{C. logarítmic} $\left( \lim\limits_{n \to \infty} \frac{-\log{a_n}}{\log{n}} = L \right) \implies$ ($L > 1$ conv.) i  ($L < 1$ div.) \\
\underline{C. condensació} ($a_n$ decreixent, $a_n \geq 0$)
    $\sum a_n$ convergent $\iff \sum 2^na_{2^n}$ convergent
...
\ci Sèrie alternada $\implies$ un pos., un neg., ... \\\u{C. Dirichlet}: Si $s_n$ d'$(a_n)$ fitades i $(b_n)$ decr., $\lim b_n = 0$, llavors $\sum a_nb_n$ convergeix. \\
