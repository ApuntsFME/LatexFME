...
\underline{C. de la integral} $\left( a_n = f(n), f \text{ integ.} \right)$ $\int_{M}^{\infty} f$ converge $\iff \sum a_k$ converge y $\sum_{M}^{\infty} = \sum_{M}^{N-1} + \int_{N}^{\infty} f + \varepsilon_N$, $\varepsilon_N \in [0,a_N]$ \\
\underline{C. logarítmico} $\left( \lim\limits_{n \to \infty} \frac{-\log{a_n}}{\log{n}} = L \right) \implies$ ($L > 1$ conv.) i  ($L < 1$ div.) \\
...
\u{Mesura nul·la}: Recobert per numerables \\ \columnbreak rectangles de mesura $< \varepsilon, \, \forall \varepsilon > 0$. \\
