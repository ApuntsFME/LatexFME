...
\begin{comment} 
    \subsection{Error absoluto y relativo de $f\lp \overline{x}\rp $ con cálculo aproximado de $f$}
    
    Supongamos que ademas del error en los datos $\overline{x} = x + e_a\lp x\rp $, obtenemos un error relativo al calcular $f$ acotado por $\delta_f$. Entonces tenemos la siguiente expresión para el error absoluto cometido:
    
    \[
            e_a\lp \overline{f}\lp \overline{x}\rp \rp  \simeq |\overline{f}'\lp \overline{x}\rp ||e_a\lp x\rp | + \delta_f|\overline{f}\lp \overline{x}\rp |
    \]
