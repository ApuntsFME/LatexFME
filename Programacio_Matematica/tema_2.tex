...
\[\theta_D^\ast = \frac{-r_{\N(q)}}{d_{r_\N(q)}}\,,\] i llavors, altre cop per 
...
\begin{alg}[del símplex dual]\label{alg:ASD}
	\begin{enumerate}
		...
		\end{itemize}
		%
		\item {\bf Càlcul de $\theta^\ast_D$}:
			\[\theta_D^* = \min_{\{j \in \N \,|\, d^D_j <
				0\}} \left\{-\frac{r_j}{d^D_j} \right\}.\]
		%
		\item VNB primal entrant: $x_q$ tal que $\theta^\ast_D =
			-\dfrac{x_q}{d_q}$.
		%
		\item {\bf Actualitzacions i canvi de base}:
		\begin{itemize}
			\item $\vb{r}_\N \coloneqq \vb{r}_\N + \theta_D^* 
			\vb{d^D_{\vb{r}_\N}}$, \\
			$\vb{\lambda} \coloneqq \vb{\lambda} - \theta_D^* 
			\beta_p^\prime$, \\
			$\vb{r}_{B\left(p\right)} \coloneqq \theta_D^*$, \\
			$z \coloneqq z - \theta^* \vb{x_{B\left(p\right)}}$.
			\item $\B \coloneqq \B \setminus \left\{B\left(p\right)\right\} 
			\cup \left\{q\right\}$, \\
			$\N \coloneqq \N \setminus \left\{q\right\} \cup 
