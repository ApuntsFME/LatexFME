...
\begin{example}
    Sea $G = \Sim_4$, $H = \setb{ \Id, (1,2)(3,4)}$ y $K = \setb{ \Id, (1,2)(3,4), (1,3)(2,4), (2,3)(1,4)}$.
    ...
    \begin{enumerate}[i)]
        ...
        \item La función
            \[
                ...
                \end{aligned}
            \]
            es un morfismo de grupos exhaustivo y de nucleo $H$.
...
\begin{teo}[del isomorfismo]
    ...
    \begin{enumerate}[i)]
        \item $\tilde{f}$ está bien definida $\iff H \subseteq \ker(f)$
            ...
            En caso de que $H \subseteq \ker(f)$, se cumple que 
...
\begin{col}
    Hay un único grupo cíclico de orden $n$ (a parte de morfismos).
\end{col}
...
\begin{proof}
    ...
    \[
        ...
        \end{aligned}
    \]
    que es un morfismo de grupos, entonces, por el teorema,
    \[
...
\begin{defi}[todo]
    Sea $G$ un grupo finito con $\orden(G) = n$ y $d \vert n$, notamos
    \[
        \mathcal{O}_d = \setb{y \in G \vert \orden(y) = d}
    \]
...
\begin{lema}
    ...
    \[
        \# \setb{\text{raices de } p(T)} \leq n
    \]
\end{lema}

\begin{lema}
    Sea $\k$ un cuerpo, sea $x \in \k^\ast$ con $\orden(x) = n$, entonces
    \[
        \O\left( \k^\ast \right) \subseteq \setb{ \text{raices de } T^n -1}
        = C_n(x) \subseteq \k^\ast
    \]
    Además, $\#\O_n\left( \k^\ast \right) \leq \varphi(n)$
\end{lema}

\begin{proof}
    Si $y \in \O_n(\k^\ast) \implies y^n = 1 \implies y^n -1 = 0 \implies y$ es raíz de $T^n -1$.
    Por lo tanto, $\O_n\left( \k^\ast \right) \subseteq{\text{raices de } T^n -1}$. Ahora, vemos que
    \[
        y \in C_n(x) \implies y = x^k \implies y^n = \left( x^k \right)^n = \left( x^n \right)^k = 1
    \]
    y $C_n(X) \subseteq \setb{\text{raices de } T^n -1}$. Por último tenemo que 
    $\# C_n(x) \leq \# \setb{\text{raices de } T^n -1} \leq n$
    %TODO
\end{proof}

\begin{teo}
    Sea $\k$ un cuerpo y sea $G$ un subgrupo finito de $\k^\ast$. Entonces $G$ es un grupo
    ciclico. En particular, si $\k$ es finito, entonces $\k^\ast$ es un grupo cíclico.
\end{teo}
...
\begin{proof}
    ...
    \[
        ...
        \subseteq \setb{y \in \k^\ast \vert \orden(y) = } = \O_d\left( \k^\ast \right)
    \]
