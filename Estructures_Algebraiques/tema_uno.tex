...
\begin{prop}[Fórmula de órbitas de la acción por conjugación de $G$ en $G$]
...
\end{teo}
\begin{proof}
    Si $G$ es un grupo abeliano, procederemos a demostrar el enunciado por inducción sobre $n$. El caso
    $n = 1$ es trivial. Suponemos $n \geq 4$ y tomamos $x \in  G$, por el teorema de Lagrange tenemos que
    $\orden(x) = m \vert n$. Separamos ahora en dos casos:

    Si $p \vert m$, entonces, $\orden\left( x^{\frac{m}{p}} \right) = p$ y hemos acabado.

    Si $p \not\vert m$, tenemos $\langle x\rangle \triangleleft G$ (por ser $G$ abeliano), por lo 
    tanto $\faktor{G}{\langle x\rangle}$ es grupo cociente. y tenemos
    \[
        \abs{\faktor{G}{\langle x \rangle}} = \frac{\abs{G}}{\abs{\langle x \rangle}} = \frac{n}{m}
    \]
    Como $p \vert n$ y $p \not\vert n$ se tiene que $p \vert \abs{\faktor{G}{\langle x \rangle}} < n$
    y por hipótesis de inducción, $\exists \bar{y} \in \faktor{G}{\langle x \rangle}$ tal que
    $\orden\left( \bar{y} \right) = p$, pero
    \[
        \begin{aligned}
            \orden\left( \bar{y} \right) = p &\implies \left( \bar{y} \right) = \bar{1} \implies
            y^p \in \langle x \rangle \implies y^p = x^r \implies
            \left( y^m \right)^p = \left( y^p \right)^m = \left( x^r \right)^m = 
            \left( x^m \right)^r = 1^r = 1
        \end{aligned}
    \]
    y por lo tanto, $\orden\left( y^m \right) \vert p$, pero $p$ es primo, con lo cual, o bien
    $\orden\left( y^m \right) = 1$ pero entonces
    \[
        \orden\left( y^m \right) = 1 \implies y^m = 1 \implies \bar{y^m} = \bar{1} \implies 
        \left( \bar{y} \right)^m = 1 \implies p \vert m
    \]
    Lo cual es una contradicción. Por lo tanto, $\orden\left( y^m \right) = p$ y hemos acabado.

    En el caso de que $G$ no sea abeliano aplicaremos la fórmula de órbitas separando en varios casos.
    
    \begin{itemize}
        \item Si $\exists x_1, \dots, x_r$ tal que $\mcd\left( p, \left[ G : G_{x_i} \right] \right) = 1$.
            Entonces $p \not\vert \left[ G : G_{x_i} \right] = \frac{\abs{G}}{ \abs{G_{x_i}}}$, pero
            $p \vert \abs{G} \implies p \vert \abs{G_{x_i}} \leq n$ y, aplicando la hipótesis de inducción
            $\exists x \in G_{x_i} \subset G$ tal que $\orden(x) = p$.
        \item Si $\forall x_1, \dots, x_r$ se tiene que $p \vert \left[ G : G_{x_i} \right] \implies
            p \vert \abs{Z(G)}$, pero $Z(G) \subset no igual G$, es decir, $\abs{Z(G)} < n$ y por lo tanto,
            por la hipótesis de inducción, $\exists x \in Z(G)$ tal que $\orden(x) = p$.
    \end{itemize}
\end{proof}
...
\begin{proof}
    \begin{enumerate}[i)]
        \item \bimplies OK. \bimpliedby Suponemos $G$ no trivial. Si $G$ es abeliano $\implies Z(G) = G$
            no trivial. Por lo tanto, $Z(G)$ es no trivial.
            Tomamos $x_1, \dots, x_s$ representantes de las órbitas no puntuales y $x_i \notin Z(G)$. En
            particular, $Z_G\left( x_i \right) \subsetneq G$ y tenemos
            \[
                \left[ G : Z_G\left( x_i \right) \right] = \frac{\abs{G}}{\abs{Z_G\left( x_i \right)}} =
                \frac{p^r}{p^{s_i}} = p^{t_i}
            \]
            con $t_i \geq 1$, ya que $t_i = 0 \implies s_i = r \implies Z_G\left( x_i \right) = G$, lo cual no
            puede ser. Por lo tanto, $p \vert \left[ G : Z_G\left( x_i \right) \right]$ y $p \vert i\abs{G}$, 
            $\implies p \vert \abs{Z(G)}$ y por lo tanto, $Z(G)$ es no trivial.
        \item Procederemos por inducción sobre $\abs{G}$. Si $\abs{G} = 1 = p^0 \implies G$ es abeliano.
            Supondremos ahora que $G$ no es abeliano y que $\abs{G} > 1 \stackrel{\ref{teo:pgrupo-1}}{\implies}$
            $Z(G)$ no trivial y $Z(G) \subsetneq G$.
            Tenemos $G \triangleright Z(G) \triangleright \left\{ 1 \right\}$. Tenemos
            $\faktor{Z(G)}{ \left\{ 1 \right\} } \cong Z(G)$ es abeliano y
            $\abs{\faktor{G}{Z(G)}} = \frac{p^r}{p^{s}}$ con $0 < s < r$, ya que $Z(G)$ es no trivial y
            $G$ no es abeliano. Aplicando ahora la hipótesis de inducción
            \[
                \faktor{G}{Z(G)} \text{ es resoluble y } Z(G) \text{ es resoluble} \implies G
                \text{ es resoluble}.
            \]
        \item Sea $G$ un $p$-grupo simple. Por \ref{item:pgrupo-2}, $\exists G = G_0 \triangleright G_1
            \triangleright \cdots \triangleright G_n = \left\{ 1 \right\}$ torre normal abeliana.
            Como $G$ es simple, no tiene subgrupos normales propios $\implies n = 1$ y
            $G \triangleright \left\{ 1 \right\}$ es una torre normal abeliana $\implies G \cong
            \faktor{G}{ \left\{ 1 \right\} }$ es abeliano simple $\implies G \cong \faktor{\z}{p\z}$.
    \end{enumerate}
\end{proof}

\section{Grupos de Sylow}

El Teorema de Lagrange \ref{teo:lag} nos dice que si $H$ es subgrupo de $G$ finito
$\implies \abs{H} \vert \abs{G}$. Lo natural, es preguntarse si el recíproco es cierto, es decir, que si
$n \vert \abs{G} \implies \exists H$ subgrupo de $G$ tal que $\abs{H} = n$. Si $n = p$ primo, es cierto,
ya que por el Teorema de Cauchy (\ref{teo:cauchy}) $\exists x \in G$ con $\orden(x) = p$ y $H = \langle x\rangle$.
Aunque en general, este enunciado es falso.
\begin{example}
    Tomamos $G = \Asim_5$, $\abs{G} = 60$. Tomamos $n = 30$. No existe ningún subgrupo $H$ con $\abs{H} = 30$, ya
    que si existiera:
    \[
        \abs{H} = 30 \implies \left[ G : H \right] = \frac{\abs{G}}{\abs{H}} = \frac{60}{30} = 2
        \implies H \triangleleft G
    \]
    Lo cual es una contradicción con que $G$ es simple.
\end{example}

\begin{teo}[Primer teorema de Sylow]
    Sea $G$ un grupo finito y sea $p$ un primo tal que $p^r \vert \abs{G}$. Entonces $\exists H$ subgrupo
    de $G$ tal que $\abs{H} = p^r$.
\end{teo}

\begin{proof}
    Realizaremos la demostración por inducción sobre $\abs{G}$. El caso base ($\abs{G} = 1$) es trivial.
    Dividiremos la demostración en dos casos:
    \begin{itemize}
        \item $p \not\vert \abs{Z(G)}$. Por la fórmula de órbitas \ref{prop:orb}, $\exists x_1,\dots,x_s$ tal que
            \[
                p \not\vert \left[ G : Z_G\left(x_i \right) \right] = \frac{\abs{G}}{\abs{Z_G\left( x_i \right)}}
                \implies p \vert \abs{Z_G\left( x_i \right)} < \abs{G}
            \]
            Y por hipótesis de inducción $\exists H$ subgrupo de $Z_G\left( x_i \right)$, y por lo tanto de $G$,
            tal que $\abs{H} = p^r$.
        \item $p \vert \abs{Z(G)}$. Por el Teorema de Cauchy (\ref{teo:cauchy}) $\exists  \in Z(G)$ tal que
            $\orden(x) = p$. Tomamos $L = \langle x \rangle$. Como $x \in Z(G) \implies L \triangleleft G$, por
            lo tanto, $\faktor{G}{L}$ es grupo y tiene orden $\abs{\faktor{G}{L}} = \frac{\abs{G}}{\abs{L}} =
            \frac{\abs{G}}{p}$. Como $p^r \vert \abs{G} \implies p^{r-1} \vert \abs{\faktor{G}{L}} < G$ y por la
        hipótesis de inducción $\exists \bar{H}$ subgrupo de $\faktor{G}{L}$ tal que $\abs{H} = p^{r-1}$.
        Pero $\bar{H}$ es subgrupo de $\faktor{G}{L} \implies \bar{H} = \faktor{H}{L}$ con $H$ subgrupo de $G$,
        $L \subseteq H$. Entonces
        \[
            p^{r-1} = \abs{\bar{H}} = \abs{\faktor{H}{L}} = \frac{\abs{H}}{\abs{L}} = \frac{\abs{H}}{p}
            \implies \abs{H} = p
        \]
    \end{itemize}
\end{proof}
