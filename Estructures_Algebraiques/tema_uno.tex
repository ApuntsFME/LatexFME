...
\begin{defi}[Acción de un grupo sobre un conjunto]
    Sea $X$ un conjunto y $G$ uun grupo. Llamamos acción de $G$ en $X$  una aplicación
    \[
        \begin{aligned}
            G \times X &\to X \\
            (a, x) &\mapsto ax
    ...
    que verifica
    \begin{itemize}
        \item $a(bx) = (ab)x$, $\forall a,b \in G$ y $\forall x \in G$
        \item $1x = x$
    \end{itemize}
    Diremos que $G$ AUX en $X$ o bien que $X$ es un $G$-conjunto.
\end{defi}
...
\begin{obs}
    ...
    \[
        ...
        \end{aligned}
    \]
    $m_a$ es biyectiva ya que $\left( m_a \right)^{-1} = m_{a^{-1}}$
    $\left( a^{-1}(ax) = \left( a^{-1}a \right)x = 1x = x \right)$.

    %TODO m es morfismo de grupos
    ...
    Reciprocamente, dada $f \colon G \to \text{Perm}(X)$ morfismo de grupos. Entonces, podemos
    definir una acción
    \[
        \begin{aligned}
    ...
    \[
        \begin{aligned}
            a(bx) &= f(a)\left( f(b)(x) \right) = \left( f(a)f(b) \right)(x) = f(ab)(x) = (ab)x \\
            qx &= = f(1)x = \Id(x) = x
        \end{aligned}
    \]

    Por lo tanto, existe una biyección
    \[
...
\begin{prop}
    Sean $x, y \in X$ diremos que $x \sim y \iff \exists a \in G$ tal que $y = ax$. Entonces
    $\sim$ es una relación de equivalencia
\end{prop}

\begin{obs}
    Si tenemos la relación de equivialencia anterior, entonces
    $\bar{x} = \left\{ y \in X \mid x \sim y \right\} = \left\{ y \in X \mid \exists a \in G \tq y = ax \right\}
    = \left\{ ax \mid a \in G \right\} = Gx =$ órbita de $x$.

    El conjunto de órbita se escribe $G \ X = \faktor{X}{\sim}$
\end{obs}

\begin{defi}[estabilizador de un elemento]\idx{grupo!de isotopía de un elemento}
    Sea $G$ un grupo y $X$ un $G$-conjunto. Llamamos estabilizador de $x \in X$ o isotopia de $x$ a
    \[
        G_x = \left\{ a \in G \mid ax = x \right\} \subset G
    \]
\end{defi}
...
\begin{proof}
    $x \sim y \implies y = ax$ para algún $a \in G$. Veremos que $G_y = aG_aa^{-1}$.

    \fbox{$\subseteq$}
    Sea $\alpha \in G_y$
    \[
        \begin{aligned}
            \alpha \in G_y &\implies \alpha y = y \implies \alpha ax = ax \implies
            \left( a^{-1} \alpha a \right)x = x \implies a^{-1} \alpha a \in G_x
            &\implies \alpha a \in a G_x \implies \alpha \in a G_x a^{-1}.
        \end{aligned}
    \]
...
\begin{prop}
    Sea $G$ un grupo y $X$ un $G$-conjunto. Entonces $\forall x \in X$, tenemos
    \[
        \begin{aligned}
            G x &\to \faktor{G}{G_x} \\
            ax &\mapsto a G_x
    ...
    En particular
    \begin{enumerate}[i)]
        \item $\abs{G_x} = \faktor{G}{G_x} = \left[ G : G_x \right] = \frac{\abs{G}}{\abs{G_x}}$. Es decir,
            $\abs{Gx} = \abs{G}$.
        \item Si $X$ es finito, entonces $X = \bigsqcup\limits^r_{i = 1} G x_i$ y por tanto
            \[
                \abs{X} = \sum^r_{i = 1} \abs{G x_i} = \sum^r_{i = 1} \left[ G : G_{x_i} \right]
            \]
            Lo que conocemos como fórmula de órbitas.
        \item 
    \end{enumerate}
\end{prop}
...
\begin{proof}
    ...
    \]
    por lo tanto es inyectiva. Y la aplicación es evidentemente exhaustiva por construcción.
\end{proof}
...
\begin{defi}[punto fijo]
    ...
    \]
    es decir, $Gx = \left\{ x \right\}$ o $G_x = \left\{ a \in G \mid ax = x \right\} = G$.
\end{defi}

\begin{defi}[acción!transitiva]
    Sea $G$ un grupo, $X$ un conjunto y $G \times X \to X$ una acción de $G$ en $X$. Diremos
    que esta acción es transitiva si $\forall x, y \in X$ se tiene que $x \sim y$, es decir,
    $\exists !$ órbita o $X = Gx$ $\forall x \in X$.
\end{defi}
...
\begin{defi}[acción!por traslación]
    Sea $G$ un grupo. Definiremos una acción por traslación como la aplicación
    \[
        \begin{aligned}
            G \times G &\to G \\
...
\begin{proof}
    Tomamos la acción de $G$ en $X=G$ por traslación. Sabemos que se trata de una acción fiel, por
    lo tanto,
    \[
        TODO %TODO
    \]
...
\begin{defi}[acción!por conjugación]
    Sea $G$ un grupo. Definiremos la acción por conjugación de $G$ como la aplicación
    \[
        \begin{aligned}
            G \times G &\to G \\
            (a, x) &\mapsto axa^{-1}
...
\begin{prop}
    Sea $G$ un grupo. Si $x \in G$ es punto fijo por conjugación, entonces $x \in Z(G)$ (centro de $G$).
\end{prop}
\begin{proof}
    $x$ punto fijo $\iff axa^{-1} \forall a \in G \iff ax = xa \forall a \in G \iff x \in Z(G)$
\end{proof}

\begin{prop}
    Sea $G$ un grupo. Si $x \in G \implies G_x = Z_G(x)$
\end{prop}
\begin{proof}
...
\begin{teo}[Teorema de Cauchy]
    Sea $G$ un grupo finito con $\abs{G} = n$ y sea $p \vert n$ un primo. Entonces
    existe $x \in G$ tal que $\orden(x) = p$.
\end{teo}
\begin{proof}
