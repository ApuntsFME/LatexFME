...
\begin{teo}[Primer teorema de Sylow]\label{teo:ptsyl}
...
\begin{defi}[Sylow de un grupo]\idx{subgrupo!$p$-Sylow}
    Sea $G$ un grupo finito con $\abs{G} = p^rm$ con $p$ primo, $r \geq 0$ y $p \nmid m$. Defiminos
    \[
        \text{Syl}_p(G) \colon = \left\{ H \text{ sg. de } G \mid \abs{H} = p^r \right\}
    \]
    Llamaremos $p$-Sylow de $G$ a cualquier $H \in \Syl_p(G)$ y notaremos como $n_p(G) = \# \text{Syl}_p(G)$
\end{defi}

\begin{obs}
    Si $p \nmid \abs{G} \implies \left\{ 1 \right\}$ es $p$-syloso y $n_p(G) = 1$. Y si
    $p \vert \abs{G}$, por el PTS (\ref{teo:ptsyl}), $n_p(G) \geq 1$.
\end{obs}

\begin{defi}[acción!por conjugación de $G$ en los subgrupos]
    Sea $G$ un grupo y sea $X = \left\{ H \mid H \text{ es sg. de } \right\}$. Definiremos la acción por
    conjugación como
    \[
        \begin{aligned}
            G \times X &\to X \\
...
\begin{obs}
    La aplicación está bien definida, ya que $aHa^{-1}$ es, effectivamente, subgrupo de $G$.
\end{obs}
...
\begin{obs}
    ...
    \]
    Y tambin $1 \cdot H = 1 H 1^{-1} = H$
\end{obs}

\begin{obs}
    Los puntos fijos son los $H$ subgrupos de $G$ tales que $aHa^{-1} = H$ $\forall a \in G$, es decir,
    los puntos fijos los $H$ tales que $H \triangleleft G$. Además para todo $H$ subgrupo, la órbita de $H$
    se escribe como
    \[
        G\cdot H = \left\{ aHa^{-1} \mid a \in G \right\} = \left\{ \text{conjugado de } H \right\}
    \]
\end{obs}

\begin{defi}[normalizador de un subgrupo en un grupo]
    Sea $G$ un grupo y $H \subseteq G$ un subgrupo. Definimos el normalizador de $H$ en $G$ como
    \[
        N_G(H) = \left\{ a \in G \vert aHa^{-1} = H \right\}
    \]
    que es un grupo de $G$.
\end{defi}
...
\begin{teo}[Segundo teorema de Sylow]\label{teo:stsyl}
    Sea $G$ un grupo finito con $\abs{G} = p^rm$ con $p$ primo y $p \nmid m$. Entonces
    \begin{enumerate}[i)]
        \item Sea $L$ un $p$-subgrupo de $G$. Entonces $\exists H$ $p$-Sylow de $G$ tal que $L \subseteq H$.
        \item Sean $H$ y $P$ dos $p$-Sylow de $G$. Entonces $H$ y $P$ so conjugados.
