...
\begin{proof}
    Ejercicio.  % TODO revisar corregir això per poder-ho penjar
    \iffalse
    \begin{enumerate}[i)]
        \item[]
...
\begin{defi}[grupo!simple]
    Sea $G$ un grupo no trivial ($G \neq \setb{1}$). Decimos que $G$ es simple si los unicos subgrupos
    normales de $G$ son $\setb{1}$ y $G$.
\end{defi}

\begin{teo*}
    Sea $G$ un grupo no trivial, entonces son equivalentes
    \begin{enumerate}[(i)]
        \item\label{item:teo-random1} $G$ es simple abeliano
...
\begin{proof}
    Veremos que \ref{item:teo-random1} $\implies$ \ref{item:teo-random2}.

    Como $G \neq \setb{1}, \, \exists x \in G$ con $x \neq 1$. Veamos que si $\orden(x) = \infty \implies
    \langle x^2 \rangle \subset G$. Si $\langle x^2 \rangle = G \implies x \in \langle x^2\rangle
    \implies x = \left( x^2 \right)^n = x^{2n} \implies x^{-1}x = x^{-1}x^{2n} \implies
    1 = x^{2n-1} \implies \orden(x) \leq 2n-1$ lo cual es una contradicción.

    Tenemos ahora $\langle x^2 \rangle$ subgrupo propio de $G$. Pero $G$ es abeliano por lo tanto
    $\langle x^2 \rangle \triangleleft G$ contradicción con que $G$ es simple.

    Por tanto, $\orden(x) = n > 1$. Tomamos $p \vert n$, con $p$ primo. Sabemos que
    \[
    ...
    Por tanto, $H$ es subgrupo de $G$ y $G$ es abeliano, es decir $H \triangleleft G$, pero $G$ es simple,
    con lo cual $H = G$ y $\abs{G} = \abs{H} = p$.

    Ahora veremos que \ref{item:teo-random2} $\implies$ \ref{item:teo-random3}. Suponemos que $\abs{G} = p$
    con $p$ primo, por lo tanto, $\exists x \in G, \, x \neq 1$. En particular $\orden(x) \vert \orden(G) = p$
    como $p$ es primo, $\orden(x) = p$. Ahora se tiene que $\langle x\rangle \subseteq G$, pero los ordenes
    de los grupos son iguales, en consecuencia $G = \langle x\rangle \equiv \faktor{\z}{p\z}$.

    Por último, veremos que \ref{item:teo-random3} $\implies$ \ref{item:teo-random1}. Se tiene
    que $G \equiv \faktor{\z}{p\z}$ que es abeliano y por lo tanto $G$ es un grupo cíclico de $p$ elementos.
    Vimos que
    \[
        \begin{aligned}
            \setb{d \mid d \vert n \; 1 \leq d \leq n} &\leftrightarrow
            \setb{H \mid H \text{ sg. de } G, \; \abs{H} = d} \\
            d &\mapsto \langle y^{\frac{n}{d}} \rangle
        \end{aligned}
    \]
    Como $n$ es primo, los únicos subgrupos de $G$ son $\setb{1}$ y $G$ y, en particular, son los únicos simples
    por lo tanto, $G$ es simple abeliano.
\end{proof}

\begin{teo}[de Feit-Thompson]
    Sea $G$ un grupo tal que $\abs{G} = n$ impar. Si $G$ es simple, entonces
    \[
        n = p \text{ primo} \quad \text{y} \quad G \equiv \faktor{\z}{p\z}
    \]
\end{teo}

\begin{teo*}
    Sea $n \geq 5$, entonces $\Asim_n$ es simple.
\end{teo*}

\begin{proof}
    Sabemos (por problemas) que $\Asim_n = \langle \text{3-ciclos}\rangle$. Sean ahora
    $\left( a_1, a_2, a_3 \right)$ y $\left( b_1, b_2, b_3 \right)$ dos 3-ciclos, veremos que
    $\exists \sigma \in \Asim_n$ tal que
    ...
    Si $\sigma \in \Asim_n$ ya hemos acabado. Si $\sigma \notin \Asim_n$, tomamos
    \[
        \tilde{\sigma} = \sigma \left( a_4, a_5 \right)
    \]
    con todos los $a_{i}$ diferentes entre ellos. ($a_4$ y $a_5$ existen porque $n \geq 5$) y se tiene que
    \[
        \begin{aligned}
            \tilde{\sigma} \left( a_1, a_2, a_3 \right) \tilde{\sigma}^{-1} &=
            \sigma \left( a_4, a_5 \right) \left( a_1, a_2, a_3 \right) \left( a_4, a_5 \right) \sigma^{-1} \\
            &= \sigma \left( a_1, a_2, a_3 \right) \cancels{\left( a_4, a_5 \right)}
            \cancels{\left( a_4, a_5 \right)} \sigma^{-1} \\
            &= \sigma\left( a_1, a_2, a_3 \right)\sigma^{-1} = \left( b_1, b_2, b_3 \right)
        \end{aligned}
    \]
    y $\tilde{\sigma} \in \Asim_n$.

    Tomamos $H \triangleleft \Asim_n$  con $H$ no trivial. Entonces $H$ contiene un 3-ciclo. Tomamos 
    $\sigma \in H$ con $\sigma \neq \Id$. Por el ejercicio 20 se tiene que
    \[
        \begin{cases}
            \exists \tau \in \Asim_n \tq \tau \sigma \tau^{-1} \sigma^{-1} \text{ es un 3-ciclo} \\
            \exists \tau_1 \tau_2 \in \Asim_n \tq \tau_2 \tau_1\sigma\tau^{-1}\sigma^{-1}\tau_2^{-1}
            \sigma\tau\sigma^{-1}\tau^{-1}
        \end{cases}
    \]

    Ahora queremos ver que $H = \Asim_n$, acabamos de ver que $\exists \sigma \in H$ que es un 3-ciclo
\end{proof}
