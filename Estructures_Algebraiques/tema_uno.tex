...
\begin{col}[Retículo de subgrupo de un grupo cíclico]
    ...
    \[
        \begin{aligned}
            \setb{d \left\vert 1 \leq d \leq n, d \vert n \right.} &\longleftrightarrow
...
\begin{obs}
    ...
    \[
        ...
        \end{aligned}.
    \]
    Que es una aplicación biyectiva ($f^{-1}_a = f_{a^{-1}}$). Notemos, que $f$ no es un morfismo
    de grupos (en general), ya que $f(1) = a \neq 1$. Se tiene también que
    \[
...
\begin{defi}[conjunto!cociente]
    Llamaremos conjunto conciente de $G$ módulo $H$ a 
    \[
        G/H = \setb{aH \vert a \in G} = \setb{ \bar{a} \vert a \in G}
    \]
...
\begin{teo}[de Lagrange]
    Sea $G$ n grupo finito y sea $H$ un subgrupo de $G$. Entonces
    \[
        \abs{g} = \abs{H} \abs{G/H}
    \]
\end{teo}
...
\begin{proof}
    ...
    Por lo tanto,
    \[
        \abs{G} = \abs{\sqcup aH} = \sum \abs{aH} = \sum^{\abs{G/H}}_{k = 1} \abs{H} = \abs{H} \abs{G/H}
    \]
\end{proof}
...
\begin{defi}
    ...
    \]
    Y Llamaremos clase lateral por la derecha de $a$ módulo $H$ a $Ha$.
\end{defi}
...
\begin{proof}
    Ejercicio: demostrar que está bien definida y es biyectiva.
\end{proof}

\begin{defi}[índice! de un grupo en un subgrupo]
    Sea $G$ un grupo y $H \subseteq G$ un subgrupo. Llamaremos índice de $G$ en $H$ al cardinal de $G/H$.
    Y lo denotaremos como
    \[
        \left[G \colon H \right] = \abs{G/H} \stackrel{\text{TL}}{=} \frac{\abs{G}}{\abs{H}}
    \]
\end{defi}
