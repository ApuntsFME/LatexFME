...
\begin{eje}
    ...
    $\norm{u}'=\norm{\phi(u)})$ lo son.
    Como cualquier $\R$-espacio vectorial de dimensión finita es isomorfo a $\R^n$
    ...
    Para ver esto ultimo, comenzamos demostrando que
    \[
    ...
    Por lo tanto, esto nos dice que si consideramos una norma $\norm{\cdot}$
    como una función en el espacio m\'etrico ordinario de $\R^n$
...
\begin{eje}
  \begin{enumerate}[(a)]
   ...
   \item $D_1\lp x,y\rp = \sup\min\lc\abs{x_n-y_n},1\rc = \sup\min\lc\abs{y_n-x_n},1\rc = D_1\lp y,x\rp$.
   \item \begin{align*}
          D_1\lp x,y\rp & = \sup\min\lc\abs{x_n-y_n},1\rc \\
           & = \sup\min\lc\abs{x_n-y_n+z_n-z_n},1\rc\\
           & \leq \sup\min\lc\abs{x_n -z_n}+\abs{y_n-z_n},1\rc \\
           & \leq \sup\min\lc\abs{x_n -z_n},1\rc + \sup\min\lc\abs{y_n-z_n},1\rc \\
